...
// includes function to determine the transitive
// closure matrix of a graph
...
void Undirected::TransitiveClosure(int **TC)
{// Compute the transitive closure matrix and
 // return it in the 2D array TC[1:n][1:n].
   int n = Vertices();
   
   // first label each vertex according to component
   int *L = new int [n+1];  // label array
   LabelComponents(L);

   // now compute TC

   // for i != j, TC[i][j] is 1 iff i and j are
   // in the same component
   for (int i = 1; i <= n; i++)
      for (int j = i+1; j <= n; j++)
         if (L[i] == L[j])
            TC[i][j] = TC[j][i] = 1;
         else TC[i][j] = TC[j][i] = 0;
   
   delete [] L;

   // TC[i][i] is 1 iff the degree of i is nonzero
   InitializePos();
   for (i = 1; i <= n; i++)
      if (Begin(i)) TC[i][i] = 1;
      else TC[i][i] = 0;
