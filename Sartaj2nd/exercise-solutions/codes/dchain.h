...
// extended chain with merge function included
...
{// Merge the sorted chains A and B.
   ...
   // special cases
   if (!A.first) {// A is empty
      first = B.first;
      B.first = 0;
      return *this;}
   if (!B.first) {// B is empty
      first = A.first;
      A.first = 0;
      return *this;}
   ...
   ChainNode<T> *ca = A.first,  // pointer to current node in A
                *cb = B.first,  // pointer to current node in B
                *lt;            // pointer to last node in *this
   // merge first node for *this
   if (ca->data <= cb->data) {// first node is from A
      first = lt = ca;
      ca = ca->link;}
   else {// first node is from B
      first = lt = cb;
      cb = cb->link;}

   // merge from A and B until one is empty
   while (ca && cb) {
      if (ca->data <= cb->data) {// A element goes first
         lt->link = ca;
         lt = ca;
         ca = ca->link;}
      else {// B element is smaller
         lt->link = cb;
         lt = cb;
         cb = cb->link;}
      }
